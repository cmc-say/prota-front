...
export class World {
    ...
    constructor(id: number, worldName: string, checkList?: CheckList[], characterList?: Character[]) {
        this.id = id;
    ...
    checkRoutine (checkId: number):boolean {
        const _checkList = this.checkList.find(c => c.id === checkId) as CheckList
        return _checkList.check()
    }

    addCheckList(checkListItem: CheckList) {
        if(this.checkList.find((item) => item.isEqualTo(checkListItem)) === undefined) {
            this.checkList.push(checkListItem);
        }
    }

    updateCheckList(checkListItem: CheckList) {
        const _checkList = this.checkList.filter((item) => !item.isEqualTo(checkListItem));
    ...
    removeCheckList(checkListItem: CheckList) {
        if(this.checkList.find((item) => item.isEqualTo(checkListItem))) {
            this.checkList = this.checkList.filter((item) => item.id !== checkListItem.id);
        }
    }
