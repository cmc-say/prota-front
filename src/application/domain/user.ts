...
export class User {
    ...
    constructor(id: number, userName: string, password: string, characterList?: Character[]) {
        this.id = id;
    ...
    updateCharacter(character: Character) {
        const updateCharacterId = this.characterList.findIndex(c => !c.isEqualTo(character));
    ...
    updateTodayComment(character: Character, message: string) {
        this.characterList.find(c => c.id === character.id)?.updateComment(message);
    }
